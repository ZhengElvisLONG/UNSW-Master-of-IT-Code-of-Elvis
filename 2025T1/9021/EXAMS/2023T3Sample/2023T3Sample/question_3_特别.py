...
def rotate(num, direction):
    if direction == "R":
        return num[-1] + num[0 : len(num) - 1]
    if direction == "L":
        return num[1 : len(num)] + num[0]
    
def fill_up(num, max_digit):
    num = str(num)
    if len(num) < max_digit:
        return '0' * (max_digit - len(num)) + num
    return num
...
def f(m, n, direction):
    ...
    max_digit = max(digit(m), digit(n))
    m, n = str(m), str(n)
    strn = fill_up(n, max_digit)
    strm = fill_up(m, max_digit)
    mn = int(strm) + int(strn)
    turn = 0
    for i in range(max_digit):
        if int(strm[i]) + int(strn[i]) >= 10:
            turn = 1
    result = fill_up(mn, max_digit + turn)
    print(f"{strm} plus {strn} equals {result}")
    time = max_digit - 1
    while time:
        strm = rotate(strm, direction)
        strn = rotate(strn, direction)
        mn = int(strm) + int(strn)
        result = fill_up(mn, max_digit + turn)
        print(f"{strm} plus {strn} equals {result}")
        time -= 1
